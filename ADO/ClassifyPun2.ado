...
cap program drop ClassifyPun2
program define ClassifyPun2, rclass
    ...
    *}}}*/
        ...
        *   Prefixes and Names for type variables {{{*/
            ...
            * --- default = 100 ---
            * NoPun     (b, a > 0 ~ . but insig.)
            replace t_spear_`frn'`2'`model' = 0 if `bp' > `pvalue'
            * Pun
    ...
    * two values (split)/*{{{*/
    if `words' == 3 {
        ...
        * 10 - SPun bbz = 0, baz = sig. Anstieg  _/
        replace t_`frn'`2'`model'  = -10     if `bbzp' > `pvalue' & `baz'  > 0         & `bazp' <= `pvalue' & t_`frn'`2'`model'  == 100
        * 10 - bbz < 0 & baz > 0, recheck single estimation bbz_div == 0 & baz_div > 0 (necessary as OLS slopes down bbz == 0 if baz > 0 for better fit)
        replace t_`frn'`2'`model'  = -10     if `bbzp'_div > `pvalue' & `baz'_div > 0 & `bazp'_div <= `pvalue' & t_`frn'`2'`model'  == 100
        * 11 - SPun bbz = flacher, baz = steiler -/
        * tail smaller means further from 0
        replace t_`frn'`2'`model'  = -11     if `bbz' > 0 & `bbzp' <= `pvalue' & `baz' > 0 & `bazp' <= `pvalue' & `bbz' < `baz' & ftest_`2'_`3'`model' <= `pvalue' & t_`frn'`2'`model'  == 100
        * 12 - SPun bbz = steiler , baz = flacher /-
        replace t_`frn'`2'`model'  = -12     if `bbz' > 0 & `bbzp' <= `pvalue' & `baz' > 0 & `bazp' <= `pvalue' & `bbz' > `baz' & ftest_`2'_`3'`model' <= `pvalue'  & t_`frn'`2'`model'  == 100
        * 13 - UPun  (ftest shows no difference in both)         //
        replace t_`frn'`2'`model'  = -13     if `bbz' > 0 & `bbzp' <= `pvalue' & `baz' > 0 & `bazp' <= `pvalue' & ftest_`2'_`3'`model' > `pvalue' & t_`frn'`2'`model'  == 100
        * 14 - SPunV \/
        replace t_`frn'`2'`model'  = -14     if `bbz'  < 0 & `bbzp' <= `pvalue'  &  `baz' > 0 & `bazp' <= `pvalue' & `bbz'_div < 0 & `bbzp'_div <= `pvalue' & `baz'_div > 0 & `bazp'_div <= `pvalue' & t_`frn'`2'`model'  == 100
        * ANTISOCIAL-----------
        * 20 - SAPun baz = 0, bbz = sig. Abfall  \_
        replace t_`frn'`2'`model'  = 20     if `bazp' > `pvalue' & `bbz' < 0 & `bbzp' <= `pvalue' & t_`frn'`2'`model'  == 100
        * 20 - bbz < 0 & baz > 0, recheck single estimation bbz_div == 0 & baz_div > 0 (necessary as OLS slopes down bbz == 0 if baz > 0 for better fit)
        replace t_`frn'`2'`model'  = 20     if `bazp'_div > `pvalue' & `bbz'_div < 0 & `bbzp'_div <= `pvalue' & t_`frn'`2'`model'  == 100
        * 21 - SAPun bbz = steiler , baz = flacher-\
        * tail smaller means further from 0
        replace t_`frn'`2'`model'  = 21     if `bbz' < 0 & `bbzp' <= `pvalue' & `baz' < 0 & `bazp' <= `pvalue' &  `bbz' > `baz' & ftest_`2'_`3'`model' <= `pvalue'  & t_`frn'`2'`model'  == 100
       * recheck left side insig bbz BUT sig pos alpha 1 ( using indepent regression as in 10)
        ...
        * 22 - SAPun bbz = flacher, baz = steiler -\
        replace t_`frn'`2'`model'  = 22     if `bbz' < 0 & `bbzp' <= `pvalue' & `baz' < 0 & `bazp' <= `pvalue' &  `bbz' < `baz' & ftest_`2'_`3'`model' <= `pvalue'   & t_`frn'`2'`model'  == 100
        * 23 - UAPun           //
        replace t_`frn'`2'`model'  = 23     if `bbz' < 0 & `bbzp' <= `pvalue' & `baz' < 0 & `bazp' <= `pvalue' & ftest_`2'_`3'`model' > `pvalue' & t_`frn'`2'`model'  == 100
        * 24 - SUPunV /\
        replace t_`frn'`2'`model'  = 20     if `bbz'  > 0 & `bbzp' <= `pvalue'  &  `baz' < 0 & `bazp' <= `pvalue' & `baz'_div < 0 & `bazp'_div <= `pvalue' & `bbz'_div > 0 & `bbzp'_div <= `pvalue' & t_`frn'`2'`model'  == 100
*}}}*/
